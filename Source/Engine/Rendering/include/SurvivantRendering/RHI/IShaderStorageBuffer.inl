...
{
    template <typename T>
    void IShaderStorageBuffer::SetData(T* p_data, const size_t p_count)
    {
        SetRawData(static_cast<const void*>(p_data), p_count * sizeof(T));
    }

    template <typename T>
    void IShaderStorageBuffer::SetSubData(const T* p_data, const size_t p_count, const ptrdiff_t p_offset)
    {
